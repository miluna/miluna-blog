...
description: Directrices y procedimientos para redactar, estructurar, traducir (ES/EN) y validar artículos técnicos en src/content/blog/. Usar siempre al crear, editar o traducir posts del blog.
---

# Blog Post Creation Skill

Este skill define las directrices de estilo, tono, estructura y flujo de trabajo para redactar, revisar y traducir artículos técnicos del blog (`src/content/blog/`).
...
El nombre del archivo (`<slug>.md`) debe ser **idéntico en ambas carpetas** para mantener la consistencia y el correcto emparejamiento entre idiomas en el enrutador i18n.
...
3. **Validación obligatoria:** Tras crear o editar cualquier archivo en `src/content/blog/`, ejecutar siempre:
   ...
   npm run build
   ```
   Comprueba que Astro genera la ruta estática correspondiente (tanto `/es/blog/<slug>/` como `/en/blog/<slug>/`) sin errores de schema Zod o tipado.
